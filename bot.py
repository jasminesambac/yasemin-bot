...
def ai_menu() -> InlineKeyboardMarkup:
    return kb([
        [("💬 AI Sohbet", "m:ai_chat")],
        [("🔎 Arama & Dosya", "m:ai_tools")],
        [("📊 Verilerime Sor", "ai:records")],
        [("⚙️ AI Ayarları", "m:ai_settings")],
        [("🔙 Geri", "m:main")],
    ])
...
def ai_chat_menu() -> InlineKeyboardMarkup:
    return kb([
        ...
        [("Groq Hızlı", "ai:groq"), ("İkisine de Sor", "ai:both")],
        [("🎙️ Sesli Sor", "ai:voice")],
        [("🔙 Geri", "m:ai")],
    ])


def ai_tools_menu() -> InlineKeyboardMarkup:
    return kb([
        [("🔍 Güncel Ara", "ai:web")],
...
def _sheets_get_all_records(sheet_name: str) -> list[dict[str, Any]]:
    """Google Sheets API'si arada geçici 500/502/503/504 hataları dönebiliyor (Google tarafında
    geçici bir yoğunluk/bakım durumu, bot koduyla ilgisi yok - biz de bunu kontrol edemeyiz).
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "m:ai":
        ...
        await edit_or_send(update, "AI", ai_menu())
        return
    if data == "m:ai_chat":
        context.user_data.pop("flow", None)
        await edit_or_send(update, "Hangi yapay zekaya sormak istersin?", ai_chat_menu())
        return
    if data == "m:ai_tools":
        context.user_data.pop("flow", None)
        await edit_or_send(update, "Arama & Dosya AI", ai_tools_menu())
        return
    if data == "m:ai_settings":
        context.user_data.pop("flow", None)
        await edit_or_send(update, "AI Ayarları", ai_settings_menu())
        return
    if data == "ai:agnes":
    ...
    if data == "qlog:cancel":
        ...
        return
    if data == "freetext:log":
        pending_text = context.user_data.pop("pending_free_text", None)
        if not pending_text:
            await edit_or_send(update, "Süresi geçmiş, tekrar yaz.", main_menu())
            return
        await try_quick_log_from_text(update, context, pending_text)
        return
    if data == "freetext:ai":
        pending_text = context.user_data.pop("pending_free_text", None)
        if not pending_text:
            await edit_or_send(update, "Süresi geçmiş, tekrar yaz.", main_menu())
            return
        await answer_free_chat(update, context, pending_text)
        return
...
async def diary_ai_summary(update: Update, context: ContextTypes.DEFAULT_TYPE, date: str) -> None:
    ...
    prompt = "\n".join(parts)
    answer = await ask_gemini_stateless(prompt) if GEMINI_API_KEY else await ask_groq_stateless(prompt)
...
async def send_ph_chart(update: Update, teneke: str) -> None:
    ...
    try:
        buffer = await asyncio.to_thread(render_line_chart, title, series, "pH")
    except Exception as exc:
...
async def ask_gemini_stateless(prompt: str) -> str:
    """Gemini'ye tek seferlik, hafızasız bir istek atar. context.user_data'daki gemini_history'yi
    okumaz ve ona hiçbir şey eklemez. İç kullanım (JSON ayrıştırma, özet, veri sorgusu vb.) için -
    böylece bu tür teknik/uzun promptlar kullanıcının asıl AI sohbet geçmişini kirletmez."""
    if not GEMINI_API_KEY:
        return "Gemini API anahtarı eksik. Render Variables içine GEMINI_API_KEY eklenmeli."
    ...
    try:
        ...
        if response.status_code == 429:
            if GROQ_API_KEY:
                return await ask_groq_stateless(prompt)
            return "Gemini kotası doldu (429 - çok fazla istek). Biraz bekleyip tekrar dene."
        ...
        return answer or "Gemini cevap döndürmedi."
    except Exception as exc:
        if GROQ_API_KEY:
            return await ask_groq_stateless(prompt)
...
async def ask_groq_stateless(prompt: str) -> str:
    """Groq'a tek seferlik, hafızasız bir istek atar (bkz. ask_gemini_stateless)."""
    if not GROQ_CLIENT:
        return "Groq ayarı eksik. Render Variables içine GROQ_API_KEY eklenmeli."
    try:
        response = await asyncio.to_thread(
        ...
        return (response.choices[0].message.content or "Groq cevap döndürmedi.").strip()
    except Exception as exc:
        return f"Groq hatası: {exc}"
...
async def ask_web_search(question: str, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        ...
        if GEMINI_API_KEY:
            return await ask_gemini_stateless(prompt)
        return await ask_groq_stateless(prompt)
    except Exception as exc:
...
async def parse_quick_log(text: str, context: ContextTypes.DEFAULT_TYPE) -> dict[str, Any] | None:
    ...
    try:
        raw = await ask_gemini_stateless(prompt) if GEMINI_API_KEY else await ask_groq_stateless(prompt)
    except Exception:
        return None
    cleaned = raw.strip()
...
async def ask_gemini(question: str, context: ContextTypes.DEFAULT_TYPE) -> str:
    ...
    try:
        ...
        if response.status_code == 429:
            if GROQ_API_KEY:
                return await ask_groq(question, context)
            return "Gemini kotası doldu (429 - çok fazla istek). Biraz bekleyip tekrar dene."
        if response.status_code == 403:
            if GROQ_API_KEY:
                return await ask_groq(question, context)
...
async def show_weather(update: Update, city: str, *, monthly: bool = False) -> None:
    ...
    try:
        if monthly:
            ...
            text += f"\n({city} artık bahçe şehrin olarak kayıtlı. Metninde \"sula\" geçen bir hatırlatma zamanı geldiğinde, o gün {city} için yağmur bekleniyorsa mesaja otomatik bir uyarı notu eklenecek.)"
        await edit_or_send(update, text, weather_menu())
    except Exception as exc:
        await edit_or_send(update, f"Hava durumu alınamadı: {exc}", weather_menu())


def free_text_choice_kb() -> InlineKeyboardMarkup:
    return kb([
        [("📝 Kayda Ekle", "freetext:log"), ("🤖 AI'a Sor", "freetext:ai")],
        [("❌ Vazgeç", "freetext:cancel")],
    ])


async def try_quick_log_from_text(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> None:
    """Serbest bir metni yapılandırılmış bir kayda çevirmeyi dener, tanırsa Onayla/İptal butonlarını gösterir."""
    if not (GEMINI_API_KEY or GROQ_API_KEY):
        await edit_or_send(update, "Bunun için AI ayarı eksik (GEMINI_API_KEY veya GROQ_API_KEY gerekli).", main_menu())
        return
    await update.effective_message.chat.send_action(ChatAction.TYPING)
    parsed = await parse_quick_log(text, context)
    if parsed:
        context.user_data["quick_log_pending"] = parsed
        await edit_or_send(update, quick_log_summary(parsed), quick_log_confirm_kb())
        return
    await edit_or_send(update, "Bunu yapılandırılmış bir kayda çeviremedim. İlgili menüden manuel ekleyebilir veya '🤖 AI'a Sor'u deneyebilirsin.", main_menu())


async def answer_free_chat(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> None:
    """Serbest bir metni genel AI sohbetine (Gemini/Groq/Agnes sırayla, hafızalı) sorar."""
    if not (AGNES_API_KEY or GEMINI_API_KEY or GROQ_API_KEY):
        await edit_or_send(update, "AI ayarı eksik.", main_menu())
        return
    await update.effective_message.chat.send_action(ChatAction.TYPING)
    if GEMINI_API_KEY:
...
async def text_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not flow:
        ...
        if not (AGNES_API_KEY or GEMINI_API_KEY or GROQ_API_KEY):
            await update.effective_message.reply_text("Lütfen menüden bir buton seç.", reply_markup=main_menu())
            return
        context.user_data["pending_free_text"] = text
        await update.effective_message.reply_text("Bunu ne yapayım?", reply_markup=free_text_choice_kb())
        return
...
async def document_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        summary = await ask_gemini_stateless(summary_prompt) if GEMINI_API_KEY else await ask_groq_stateless(summary_prompt)
...
async def show_stock_report(update: Update, name: str) -> None:
    ...
    if plt is not None and len(points) >= 2:
        try:
            buffer = await asyncio.to_thread(render_line_chart, f"{material} Kullanım Grafiği", {material: points}, "Miktar")
